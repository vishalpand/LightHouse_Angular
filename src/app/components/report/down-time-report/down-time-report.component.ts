...
@Component({
  selector: 'app-down-time-report',
  templateUrl: './down-time-report.component.html',
  styleUrls: ['./down-time-report.component.css']
})
export class DownTimeReportComponent {
  toDayDate:any
  ...
  getAlldownTimelist:any


  applicationList:any;
  constructor(private dashboardService: DashbordService, private cdr: ChangeDetectorRef, private spinner: NgxSpinnerService,private downtime:DowntimeService) {
  ...
  ngOnInit(): void {
    ...
    let starT = '00:00:00';
    let endT = '23:59:59';
    this.getDowntime(applicatiname, startD, endD, starT, endT);
   

  }
  getDowntime(appName: any, startD: any, endD: any, starT: any, endT: any) {
    this.spinner.show();
    startD = startD + ' ' + starT;
    endD = endD + ' ' + endT;

    this.downtime.getdownTimeData(appName, startD, endD)
    .subscribe((response: any) => {
      setTimeout(() => {
        this.spinner.hide();
      }, 1000);

      this.getAlldownTimelist = response.serviceResponse;
      console.log('dowantime response:', response);
